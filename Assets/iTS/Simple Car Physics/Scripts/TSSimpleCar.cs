...
public class TSSimpleCar : MonoBehaviour
{
	...
	*/
    ...
    IEnumerator LightBlinking(GameObject blinkingLight)
    {
        ...
        while (blinking)
        {
            if (blinkingLight.activeInHierarchy)
        ...
        if (!crashed)
        {
            ...
            else if (!superSimplePhysics)
            {
                ...
                if (playerControlled)
                {
                    ...
                    this.maxSteering = 55f;
                    float steerAngle = this.maxSteering * Mathf.Clamp(steering, -1f * maxSteering, 1f * maxSteering);
    ...
    public void OnAIUpdate(float steering, float brake, float throttle, bool isUpSideDown)
    {
        ...
        float steerAngle = maxSteering * Mathf.Clamp(steering, -1f, 1f);
    ...
    Vector3 localspeed;
    public float maxSteering = 55f;

    /// <summary>
    /// Updates the car speed.
    /// </summary>
    /// <param name="carSpeed">Car speed.</param>
    public Vector3 UpdateCarSpeed()
    {
