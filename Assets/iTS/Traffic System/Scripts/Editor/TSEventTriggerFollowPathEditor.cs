...
public class TSEventTriggerFollowPathEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        currentSelection =
            GUILayout.Toolbar(currentSelection, new string[] { "Spawning Point", "Ending Point", "AI Path" });
        ...
        {
            EditorGUILayout.HelpBox("Please make your colliders attached to this game object be triggers!",
                MessageType.Warning);
        }
    }
    ...
    void OnSceneGUI()
    {
        ...
        foreach (int lane in tempLanes)
        {
            ...
            {
                CheckPoints(ref trigger.eventEndingPoint, manager.lanes[lane].points, rayHit, hit, range, false, lane,
                    -1);
            }
            else
            {
            ...
            if (manager.lanes[lane].connectors.Length > 0)
            {
                for (int p = 0; p < manager.lanes[lane].connectors.Length; p++)
                {
                    if (currentSelection == 0)
                    {
                        CheckPoints(ref trigger.startingPoint, manager.lanes[lane].connectors[p].points, rayHit, hit,
                            range, true, lane, p);
                    }
                    else if (currentSelection == 1)
                    {
                        CheckPoints(ref trigger.eventEndingPoint, manager.lanes[lane].connectors[p].points, rayHit, hit,
                            range, false, lane, p);
                    }
                    else
                    {
    ...
    void CheckPoints(ref TSEventTrigger.TSPointReference currentPoint, TSPoints[] points, bool rayHit, RaycastHit hit,
        float range, bool isConnector, int lane, int connector)
    {
        ...
        for (int w = 0; w < points.Length; w++)
        {
            ...
            if ((points[w].point - trigger.transform.position).sqrMagnitude < range)
            {
                ...
                point.lane = lane;
                point.point = w;
                if (rayHit && (new Vector3(points[w].point.x, hit.point.y, points[w].point.z) - hit.point).magnitude <=
                    0.5f)
                {
                    if ((Event.current.type == EventType.MouseDown) && Event.current.button == 0 && !isConnector)
                    {
                        if (currentPoint.lane == point.lane && currentPoint.connector == point.connector &&
                ...
                {
                    if (currentPoint != null && currentPoint.lane == point.lane &&
                        currentPoint.connector == point.connector && currentPoint.point == point.point)
                    {
    ...
    TSEventTrigger.TSPointReference currentPath = new TSEventTrigger.TSPointReference();

    void CheckPointsPath(TSPoints[] points, bool rayHit, RaycastHit hit, float range, bool isConnector, int lane,
        int connector)
    {
        int controlID = GUIUtility.GetControlID(FocusType.Passive);
//		bool isInRange = false;
        for (int w = 0; w < points.Length; w++)
        {
            currentPath.connector = connector;
            currentPath.lane = lane;
            ...
            if (rayHit && (points[w].point - hit.point).magnitude <= manager.resolutionConnectors / 2f)
            {
                ...
                {
                    var newPath = new TSEventTrigger.TSPointReference()
                    {
                        lane = lane,
                        connector = connector
                    };
                    ...
                    var index = trigger.carPredefinedPath.FindIndex(path => path.connector == connector && path.lane == lane);
                    if (index == -1)
                    {
                    ...
                    {
                        trigger.carPredefinedPath.RemoveAt(index);
                    }
            ...
            if (trigger.carPredefinedPath.Exists(path => path.connector == connector && path.lane == lane))
            {
                Handles.color = connector == -1 ? Color.yellow : Color.magenta;
                Handles.Label(points[points.Length / 2].point,
                    connector == -1 ? "Lane " + lane : "Connector " + connector, EditorStyles.whiteLargeLabel);
            }
            else
            {
                Handles.color = connector == -1 ? Color.green : Color.grey;
            }
...
//		if (isInRange){
        ...
        if (points.Length > 4 && !isConnector)
        {
            ...
            Quaternion tempDir = Quaternion.LookRotation(
                (points[index + 1 < manager.lanes[lane].points.Length ? index + 1 : index].point -
                 points[index].point));
